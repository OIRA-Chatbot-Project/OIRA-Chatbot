...
def load_google_docs() -> List[Document]:
    ...
    os.makedirs(config.GOOGLE_DOCS_CACHE_DIR, exist_ok=True)
    ...
    for entry in entries:
        ...
        cache_path = os.path.join(config.GOOGLE_DOCS_CACHE_DIR, safe_base)

        try:
            if os.path.exists(cache_path) and not config.GOOGLE_DOCS_REFRESH:
